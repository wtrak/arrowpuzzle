...
let history=[],future=[],restoring=false,finished=false;
...
function elapsed(){const parts=(timer.textContent||'0:00').split(':').map(Number);return ((parts[0]||0)*60+(parts[1]||0))*1000}
...
function showRestored(){if(!message)return;const text=tr().restored;message.textContent=text;message.classList.add('show');setTimeout(()=>{if(message.textContent===text)message.classList.remove('show')},1800)}
...
const originalReset=reset.onclick;reset.onclick=e=>{if(restoring)return originalReset&&originalReset.call(reset,e);const a=snap();if(active(a)&&!window.confirm(tr().confirm))return;localStorage.removeItem(key);history=[];future=[];finished=false;const out=originalReset&&originalReset.call(reset,e);setTimeout(()=>{history=[snap()];buttons()},0);return out};
if(check){check.addEventListener('click',()=>setTimeout(()=>{if(win&&win.classList.contains('show')){finished=true;localStorage.removeItem(key);buttons()}},0))}
...
const initial=snap();if(saved&&Array.isArray(saved.cells)&&saved.cells.length===16){restore(saved.cells);history=[saved.cells.slice()];showRestored()}else history=[initial];
labels();buttons();
})();
